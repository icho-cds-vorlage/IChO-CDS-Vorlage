\documentclass[./main.tex]{subfiles}
\graphicspath{{\subfix{./Abbildungen/}}}
\begin{document}

\opt{icho}{
\thispagestyle{empty}
\vspace*{3cm}
\begingroup\centering\bfseries
\LARGE\opt{rd1}{Klausur zur 1. Runde}\opt{rd2}{Klausur zur 2. Runde}\opt{rd3}{Klausur zur 3. Runde}\opt{rd4}{\opt{ex1}{Theorie-Klausur}\opt{ex2}{Praxis-Klausur} zur 4. Runde} des Auswahlverfahrens zur \\\fullname\\\bigbreak
\Large Name, Vorname: \opt{c0}{\rule{6cm}{.4pt}}\opt{c1}{\suslastname, \susfirstname} \\
Teilnehmer-Code: \opt{c0}{\rule{6cm}{.4pt}\opt{rd2}{\opt{s1}{\textcolor{red}{Dummy-QR-Codes!!!}}}}\opt{c1}{\suscode} \\\bigbreak
Beginne erst, wenn das Startsignal gegeben wird.\\\bigbreak
\endgroup
\begin{table}[H]
    \centering
    \begin{tabular}{ll}
    \textbf{Zeit} & \opt{rd2}{\SI{180}{\minute}}\opt{rd3}{\SI{300}{\minute}}\opt{rd4}{\SI{300}{\minute}} \\
    \textbf{Name} & \opt{c0}{Schreibe}\opt{c1}{Kontrolliere} deinen Namen auf diese\opt{c0}{s}\opt{c1}{m} Deckblatt. \\
    \textbf{Teilnehmer-Code} & \opt{c0}{Schreibe}\opt{c1}{Kontrolliere} deinen Code auf diese\opt{c0}{s}\opt{c1}{m} Deckblatt und auf jede\opt{c1}{r}\\&  Seite der Klausur.\\
    \textbf{Atommassen} & Benutze nur das vorgegebene Periodensystem. \\
    \textbf{Konstanten} & Benutze nur die Werte aus der Formelsammlung. \\
    \textbf{Berechnungen} & Schreibe diese in die zugeh\"origen K\"asten, ohne Rech-\\& nungen gibt es keine Punkte. \\
    \textbf{Ergebnisse} & Schreibe nur in die zugeh\"origen K\"asten in der Klausur,\\& nichts anderes wird gewertet. \\
    \opt{rd2}{\textbf{Ersatzantwortb\"ogen} & Nutze ein leeres Blatt und schreibe deinen Teilnehmer-\\& Code und die Aufgabennummer darauf. \\}
    \textbf{Schmierpapier} & Benutze die freien R\"uckseiten; das dort Geschriebene \\& wird nicht bewertet. \\
\end{tabular}
\end{table}
\medbreak
{\Large\centering\bfseries Viel Erfolg!\\}

\backgroundsetup{angle = 0, opacity = 1.0, scale = 1,
   contents = {\includegraphics[width = \paperwidth]
   {IChO_Grafik_Kopf_Fusszeile.pdf}}}
\newpage
\backgroundsetup{angle = 0, opacity = \opt{s0}{1.0}\opt{s1}{0}, scale = 1,
   contents = {\includegraphics[width = \paperwidth]
   {IChO_Grafik_Kopf_Fuss_kurz.pdf}}}

}

\clearpage
   
\end{document}
